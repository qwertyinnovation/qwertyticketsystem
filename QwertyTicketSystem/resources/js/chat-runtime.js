...
const setupEcho = async () => {
    ...
    if (!echoBootstrapPromise) {
        echoBootstrapPromise = Promise.all([
            import('laravel-echo'),
            import('pusher-js'),
            ...
            echoBootstrapPromise = null;
            throw error;
        });
    }

    return echoBootstrapPromise;
};

export const initChatRuntime = async () => {
    ...
    try {
        echoInstance = await setupEcho();
    } catch (error) {
        console.error('Failed to initialize realtime chat runtime.', error);
    }
    ...
    if (broadcastChannelName !== '' && echoInstance) {
        echoInstance.private(broadcastChannelName)
    ...
    if (presenceChannelName !== '' && echoInstance) {
        presenceChannel = echoInstance.join(presenceChannelName)
            .here((users) => {
                onlineUserIds.clear();
                users.forEach((user) => {
